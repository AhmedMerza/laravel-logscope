...
A key is folded to its **ASCII nearest** before it is matched, so a field name differing from a configured entry only by an accent or a Latin-lookalike letter still redacts: `sécret` and `pаssword` (that `а` is Cyrillic) both match the ASCII defaults. Entries fold the same way, so one written in its own script matches keys spelled either way — `contraseña` and `contrasena` are one entry, and a fully non-Latin entry such as `пароль` works too.

What the fold reaches is a character that **has** a Latin form. One that has none keeps acting as a word separator, which is what it did before the fold existed — so a name written entirely in CJK (`密码`), or in a compatibility form such as fullwidth `ｐａｓｓｗｏｒｄ`, circled, or mathematical letters, is not matched by a fragment, whether it appears as a key or in `sensitive_keys`. The fold is spelling and not meaning, too: `numéro` reads as `numero`, which `card_number` does not cover.
