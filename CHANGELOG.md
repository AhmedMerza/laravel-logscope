...
- **`php artisan migrate` no longer fails when `LOGSCOPE_TABLE` is set** (#42). `2026_04_28` (the `ip_address` index, added in v1.5.3) used the table name `log_entries` instead of `logscope.table`. It failed with `no such table`, and no later migration ran, including the `user_id` change (#26). Run `php artisan migrate` after upgrading. On Postgres, log writes wait while `2026_04_28` builds its index, so on a large table run `migrate` at a quiet time (#48). MySQL builds the index without blocking writes.
