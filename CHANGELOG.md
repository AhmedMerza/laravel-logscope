...
- **Repeated entries roll up into groups, and triage moves to the group** (#29). **Behaviour change** — status and notes now belong to the group rather than to the individual entry. Marking something Resolved used to apply to exactly one row, which is fine in development and useless in production: an error that fires three thousand times is three thousand rows, and resolving one of them leaves 2,999 untouched and the list still full. The triage workflow — the most distinctive thing LogScope has — did not survive real volume.

  Every entry now carries a `fingerprint`, and entries sharing one are the same issue. The fingerprint comes from the exception in context when there is one (class, file and line), and otherwise from the message with its varying parts removed — integers, UUIDs, hex strings, IP addresses, timestamps and quoted substrings — so `User 4192 not found` and `User 87 not found` land in the same group. A new `log_groups` table holds one row per issue, with the occurrence count, when it was first and last seen, and the status and note.

  Identifiers welded into a word are covered too, which matters more than it sounds: PCRE counts `_` as a word character, so `order_4192` has no word boundary before its digits and a plain integer rule cannot see them at all. Digits adjacent to an underscore are normalised, as is any run of four or more digits anywhere in a word, so `order_4192`, `user4192` and `abc1234` each collapse. Four is the threshold on purpose — every technical token that has to survive intact carries three digits or fewer, so `sha256` is never confused with `sha512`, and `base64`, `utf8`, `oauth2`, `h264` and `x86` are left alone. The known cost is spec names: `rfc3339` and `rfc7231` both normalise to `rfc<num>` and would share a group. That was accepted rather than overlooked, because `--recompute` below makes the rule reversible.

  **A resolved group that fires again reopens itself.** This is the part worth reading twice. With status on the group, resolving an error would otherwise hide every *future* occurrence of it too — so a payment bug you closed on Tuesday could start firing again on Wednesday and LogScope would answer with a clean dashboard. That is worse than the flat list it replaces, because the flat list at least showed you the rows. Instead, a new occurrence on a Resolved group sets it back to Open and stamps `regressed_at`, so it returns to the list marked as having come back rather than as something nobody has looked at yet. **Ignored** is now the status that means permanent silence: an Ignored group is never reopened. That is the whole difference between the two closed statuses, where previously they were near-synonyms. Set `LOGSCOPE_GROUPING_REGRESSION=false` to turn the reopening off.

  Grouping costs three statements per batch flush regardless of batch size — an `insert or ignore` for fingerprints not seen before, one `update` carrying every count in a `case` expression, and one `update` to reopen anything that regressed. A 400-entry flush and a 5-entry flush both cost four queries in total, the bulk entry insert included; the batching that `LogBuffer` exists to provide is not undone. Group bookkeeping is also deliberately outside the insert's savepoint and cannot fail a write: the entries are the record, the counts are derived from them, and a group that failed to update is a wrong number that `logscope:doctor` reports and the backfill repairs — worth far less than the logs it would otherwise have discarded.

  **Upgrading:** run `php artisan migrate`, then `php artisan logscope:backfill-fingerprints` to fingerprint existing rows and build their groups. The backfill is chunked and resumable — it only looks at rows that still have no fingerprint, so an interrupted run resumes with no state to keep, and it recomputes counts rather than incrementing them, so running it twice is a no-op. Entries you had already triaged one row at a time are rolled up by taking the status of the most recently changed entry in each group, which is the last decision anyone actually made about that error; notes stay on the entries, since merging free text from many rows would invent a note nobody wrote. A group that already carries its own triage is never overwritten by a later backfill. Until the backfill runs, `logscope:doctor` reports how many entries are still unfingerprinted.

  The normalisation rules are in effect a data format: changing them changes which entries count as the same issue, so rows fingerprinted under the old rules would quietly stop matching rows written under the new ones. `php artisan logscope:backfill-fingerprints --recompute` exists for that — it re-fingerprints every row rather than only the ones with none, rebuilds the groups, and deletes the groups the old fingerprints left empty, warning you how many it removed. Triage recorded against a fingerprint that no longer exists cannot be carried over, since the rule change is precisely what re-decided which entries are the same issue; the command says so rather than dropping the rows quietly.

  `'grouping.enabled'` is a **view** setting, not a capture setting: fingerprints and group rows are written either way, and turning it off only makes the flat entry list the default. That is deliberate — status lives on the group, so a mode in which groups did not exist would need a second, parallel status implementation for the flat list, and the two would drift apart. Groups do not outlive their entries: `logscope:prune` deletes any group whose last entry it removed, in one statement after the chunked delete, so retention stays a single concept and there is no second table growing behind it.
...
- **Custom statuses declared in config now actually work.** The `statuses` config block has always documented adding your own status — the example in the published config file adds a `waiting` one — but setting it returned a 500. `getValidStatuses()` accepted the value, and `LogEntry::setStatus()` then passed it to `LogStatus::from()`, which throws on anything outside the four built-in cases. Reading was broken in the same way and for the same reason: the `status` column was cast straight to the enum, so a row holding a custom status threw a `ValueError` the moment it was loaded — meaning even a value written directly to the database took the dashboard down rather than just the write.

  The column now casts through `LogStatusCast`, which returns a `LogStatus` for the four built-in values and the plain string for a custom one, so `$log->status === LogStatus::Resolved` keeps working exactly as before. "Is this status closed?" moved to `LogStatus::isClosedValue()`, which reads the `closed` flag out of config for custom statuses and defaults to open when they don't declare one — the safer default, since a status wrongly treated as closed hides logs from the default filter. Validation moved to `LogStatus::valueOf()`, which rejects a status nothing declares with a clear `InvalidArgumentException` instead of the enum's bare `ValueError`, and the whole vocabulary — built-ins plus config — now comes from `LogStatus::allValues()` so the layer that accepts a status and the layer that stores it cannot disagree again.

- **LogScope no longer writes inside your application's database transaction** (#45). It writes on your connection, so a log written inside one of your transactions sat there uncommitted until you committed — and that had teeth. The Clear button and `logscope:prune` blocked on your request for up to `innodb_lock_wait_timeout` (50 seconds by default). Worse, a Clear covering two or more levels or channels could deadlock with a transaction that logged at two of them: MySQL kills the transaction that did less work, which is the application's, so a user's request lost its writes because somebody pressed Clear in the log viewer. Reproduced on MySQL 8.4 at `REPEATABLE READ` with 3,000 rows — enough for the planner to use the level index. And because the rows were part of your transaction, a rollback took them with it, discarding the logs that explain the rollback.

  A log written while a transaction is open is now held in memory and written as soon as that transaction ends, committed or rolled back, keeping the time it was logged rather than the time it was written. Reviewing logs cannot affect the application. This covers `sync` mode, `capture => channel` (which ignores `write_mode` entirely and always wrote immediately), `queue` mode on the `sync` or `database` driver, and the fallback row LogScope writes when a log write fails; `batch` mode already behaved this way. A broker queue (redis, sqs) is left alone, since your transaction never touched it. Entries held during a transaction are written directly when it ends rather than dispatched — the job would have written the same rows on the same connection a moment later. Set `LOGSCOPE_DEFER_IN_TRANSACTIONS=false` to restore the old behaviour.
...
`log_entries` table in clear, and each one has been true of every release up to
and including v2.0.0. There is no migration — upgrading is a composer update.
...
- **A compound `sensitive_keys` entry now redacts a name split across array levels** (#80). `Log::error('payment failed', ['card' => ['number' => $number]])` stored the card number in clear, and so did `?card[number]=4111` on a logged `Request` or URL — while the flat `card_number`, `card.number` and `cardNumber` spellings all redacted. `card[number]` is what Laravel makes of a bracketed form field, and the shape Stripe and Braintree hand a card object back in, so it is an ordinary way for a card number to reach the log table. The matcher only ever saw the leaf key, and `number` alone contains no fragment; all four multi-word defaults were affected — `card_number`, `credit_card`, `api_key`, `password_confirmation` — while one-word entries such as `password` worked at any depth already. The ancestor keys now travel down with the recursion, and are matched **only** by the multi-word half: `['card' => ['number' => …]]` reads as `cardnumber` and redacts, while `['class' => ['name' => …]]` is kept, because testing `ssn` against a joined path is exactly the gluing the word/phrase split exists to prevent. A numeric key is stepped over rather than joined, so a list of cards still matches — `is_numeric()` rather than an integer check, because PHP casts an array key to int only when it is a canonical decimal, leaving `card[01][number]`, `card[+1][number]` and `card[1.0][number]` as strings that would otherwise split the compound in half. The context key a `Request` is logged under reaches the request's own fields too, so `['api' => $request]` with `?key=…` redacts exactly as `['api' => ['key' => …]]` and `['api' => $dto]` do — in the entry's `url` as well as its `query` and `input`, since redacting one and not the other leaves the secret readable a field away. A multi-word entry in `sensitive_keys_except` still cancels only the key it names, not the same name split across levels.

  The two halves must be **adjacent** — only a numeric position is stepped over, so `['card' => ['holder' => ['number' => …]]]` does not match any more than the flat `card_holder_number` does. List `number` if an app needs that shape covered.

  **Breaking only if you subclassed `ContextSanitizer`** and overrode `sanitizeUrl()`. The url redaction moved into a new `protected redactUrl(string $url, string $path)`, and the expansion of a `Request` in logged context now calls that rather than the public method, so such an override no longer applies on that one path — override `redactUrl()` instead. `sanitizeUrl()` itself, and `ContextSanitizerInterface`, are unchanged: adding the argument to the interface would have broken every third-party implementation of it. Subclassing was never documented as an extension point; the interface binding is.

  Pre-existing — every release before this one missed the nested form the same way, so nothing that was redacted before stops being redacted. Existing rows are not rewritten.
...
- **`sensitive_keys_except` is new: keys that look sensitive but are not** (#77). Matching is deliberately broad, because a missed secret is invisible while an over-redacted field is obvious and one config line away from fixed. The exclusion list is that config line, and it ships with the LLM-era fields `token` would otherwise catch. **Your entries add to those defaults** rather than replacing them, unlike `sensitive_keys` — an app naming one of its own fields should not have to re-list LogScope's. Reach for this rather than narrowing `sensitive_keys`: narrowing the key list is what silently stops redacting something.
  ...
  The shipped exclusions cover the usage counters of the common LLM APIs — `prompt_tokens`, `completion_tokens`, `total_tokens`, `input_tokens`, `output_tokens`, `max_tokens`, `tokens_used` — plus `token_count` and `tokenizer`. Covering them all matters more than it looks: a developer who sees four counters redacted reaches for the exclusion list and adds the one word that covers them, `tokens`, which removes every word containing it and silently stops `access_tokens` being redacted — the exact regression this issue was opened about. Prefer the full field name when adding your own, for the same reason.
...
- **A configured `sensitive_keys` that is not a list of strings no longer switches redaction off** (#77). Three shapes seen in real config each produced a matcher that redacted nothing, silently: a single entry holding the whole comma-separated list (`env()` left un-exploded, or the defaults pasted out of this file's own comment), an associative flag map like `['password' => true]` whose values were read rather than its keys, and any non-string entry. Non-strings are now dropped, comma-joined entries are split, and a list that ends up with nothing usable falls back to the defaults.

- **A fragment split by a camelCase boundary inside it is matched again** (#77). `passWord` splits to `pass` + `word`, which no single word contains, so it was stored in the clear where every previous release redacted it. A run of words joined and compared for **equality** recovers those without reopening the gluing — `class_name` joins to `classname`, which contains `ssn` but does not equal it. The same rule means a configured entry works whichever way it is spelled: listing `cardnumber` now also covers `card_number` and `cardNumber`.
