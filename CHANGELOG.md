...
- **A failed log write inside a database transaction no longer rolls back the app's own writes on Postgres** (#40). LogScope writes on the app's connection. When that insert failed (a constraint, a timeout, a value too long for its column), Postgres aborted the app's whole transaction. LogScope caught the error, so `DB::commit()` returned normally and nothing reported that the app's writes were gone. This affected `sync` and `queue` write modes (queue only with the `sync` queue driver), `capture => channel`, fallback rows, and batch flushes that ran during a transaction. Each LogScope write inside a transaction now runs in its own savepoint, so a failure undoes only the log row. Writes outside a transaction are unchanged. Inside one, each write costs one extra statement for the savepoint. On Postgres, more than 64 LogScope writes in one transaction overflow Postgres's subtransaction cache, which can slow other queries while that transaction is open — the same cost as nesting `DB::transaction()` that many times.

  **A log call can now throw in one case: when the database has already ended the app's transaction** (a MySQL deadlock on the log insert, or a lost connection). LogScope can't save that transaction's writes. Hiding the error let the app keep running with each later query committed on its own, then fail at `DB::commit()` with `There is no active transaction`, its writes half committed. `Log::info()` now throws the original database error instead, as if the app's own query had deadlocked, and `DB::transaction($callback, attempts: 3)` retries the closure. Every other write failure is still caught and reported as before. Flushes at request end or PHP shutdown still never throw.
